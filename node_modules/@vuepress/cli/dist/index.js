...
import { debug as debug2, formatMs, fs as fs5, logger as logger3, withSpinner } from "@vuepress/utils";
...
var resolveCliAppConfig = (sourceDir, cliOptions, cwd = process2.cwd()) => {
  const source = path2.resolve(cwd, sourceDir);
  const appConfig = {
    source
  };
  OPTIONS_COMMON.forEach((name) => {
    if (cliOptions[name] !== void 0) {
      appConfig[name] = cliOptions[name];
    }
  });
  OPTIONS_DIRECTORY.forEach((name) => {
    if (cliOptions[name] !== void 0) {
      appConfig[name] = path2.resolve(cwd, cliOptions[name]);
    }
  });
  return appConfig;
};

// src/config/resolveUserConfig.ts
...
// src/config/resolveUserConfig.ts
var log = debug("vuepress:cli/resolveUserConfig");
var resolveUserConfig = async ({
  cliOptionsConfig,
  cliAppConfigSource
}) => {
...
// src/commands/build.ts
var log2 = debug2("vuepress:cli/build");
var build2 = async ({
  defaultAppConfig,
  sourceDir = ".",
  cliOptions = {}
  ...
  if (appConfig === null) {
    throw logger3.createError("Failed to resolve config");
  }
  ...
  logger3.success(`VuePress build completed in ${formatMs(Date.now() - start)}!`);
};

// src/commands/dev/dev.ts
import process8 from "process";
import { createDevApp } from "@vuepress/core";
import { debug as debug3, fs as fs6, logger as logger6, withSpinner as withSpinner2 } from "@vuepress/utils";

// src/commands/dev/watchPageFiles.ts
import { colors as colors3, logger as logger4, picomatch } from "@vuepress/utils";
...
var watchPageFiles = (app) => {
  ...
  const pagePatternsMatchers = app.options.pagePatterns.map(
...
// src/commands/dev/dev.ts
var log3 = debug3("vuepress:cli/dev");
var dev = async ({
  defaultAppConfig,
  sourceDir = ".",
  cliOptions = {}
}) => {
  log3(`cliOptions:`, cliOptions);
  ...
  if (appConfig === null) {
    throw logger6.createError("Failed to resolve config");
  }
  ...
  if (cliOptions.cleanCache === true) {
    await withSpinner2("Cleaning cache")(async () => fs6.remove(app.dir.cache()));
  }
...
var cli = (defaultAppConfig = {}) => {
  ...
  ).option("-t, --temp <temp>", "Set the directory of the temporary files").option("--cache <cache>", "Set the directory of the cache files").option("--clean-temp", "Clean the temporary files before build").option("--clean-cache", "Clean the cache files before build").option("--debug", "Enable debug mode").action(
    async (sourceDir, cliOptions) => build2({
      defaultAppConfig,
      sourceDir,
      cliOptions
    })
...
// src/defineUserConfig.ts
var defineUserConfig = (config) => config;
export {
  build2 as build,
  cli,
  createPageDepsHelper,
  defineUserConfig,
  dev,
  handlePageAdd,
  handlePageChange,
